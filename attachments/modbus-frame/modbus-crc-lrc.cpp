...
// CRC: https://github.com/stephane/libmodbus/blob/master/src/modbus-rtu.c

/* Table of CRC values for high-order byte */
static const unsigned char table_crc_hi[] = {0x00, 0xC1, 0x81, 0x40, 0x01, 0xC0, 0x80, 0x41, 0x01, 0xC0, 0x80, 0x41, 0x00, 0xC1, 0x81, 0x40, 0x01, 0xC0, 0x80, 0x41, 0x00, 0xC1, 0x81, 0x40, 0x00, 0xC1,
...
static const unsigned char table_crc_lo[] = {0x00, 0xC0, 0xC1, 0x01, 0xC3, 0x03, 0x02, 0xC2, 0xC6, 0x06, 0x07, 0xC7, 0x05, 0xC5, 0xC4, 0x04, 0xCC, 0x0C, 0x0D, 0xCD, 0x0F, 0xCF, 0xCE, 0x0E, 0x0A, 0xCA,
...
static unsigned short crc16(unsigned char* buffer, unsigned short buffer_length)
{
	...
	unsigned short i; /* will index into CRC lookup */
...
// LRC: https://en.wikipedia.org/wiki/Longitudinal_redundancy_check
unsigned char lrc(unsigned char* data, unsigned short len)
{
...
int main()
{
	// CRC和LRC测试样例来源：https://www.modbustools.com/modbus.html
	// 参考Function 04 (04hex) Read Input Registers内的表格
	// 需要注意：CRC在Modbus封包时字节序是小端序，即低位在前，高位在后

	unsigned char bytes[] = {0x01, 0x04, 0x00, 0x00, 0x00, 0x02};

	assert(crc16(bytes, sizeof(bytes)) == 0xcb71);
	std::cout << "OK CRC!\n";

	assert(lrc(bytes, sizeof(bytes)) == 0xf9);
	std::cout << "OK LRC!\n";
